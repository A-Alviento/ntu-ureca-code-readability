...
f2 <- function(x) {
  ...
  return(as.data.frame(relief_scores))
  ...
  #return (c(confusion_mat$overall['Accuracy'], confusion_mat$byClass['F1'], as.vector(final_model$finalModel["coefficients"])))
}
...
z = y/1000
colnames(z) = c("Relief_score")
rownames(z) = c("numLines", "avgCharsLine","ratioComment", "ratioBlank", 
                "avgNewId", "avgIdLen", "ratioIndentNumLines", "commentReadabilityFK")

# Create bar chart using ggplot
ggplot(z,aes(rownames(z), Relief_score))+geom_bar(stat="identity") 
